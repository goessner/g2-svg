...
g2.State = {
   ...
   prototype: {
      ...
      add: function(args) {
         var ifcState = g2.State[this.ifc], m2, val, trf = {}, lwnosc;
         for (var m in args) {
            val = args[m];
            ...
            if (m === "x" || m === "y" || m === "scl" || m === "w")  // transform ..
               trf[m] = val;
            else if (m === "lwnosc")  // wait ...
               lwnosc = this._current[m] = val;
         ...
         if (lwnosc !== undefined && ifcState["lwnosc"])
            ifcState["lwnosc"].call(this.ctx,lwnosc,this);
      },
...
g2.State.c2d = {
   ...
   "lw": function(val,state) { this.lineWidth = val/(state.get("lwnosc") ? state.trf.scl : 1); console.log("lw="+ val + " / " + this.lineWidth + " / " + state.get("lwnosc") + " / " + state.trf.scl)},
   ...
   "trf": function(t,state) {
             ...
             if (state.get("lwnosc")) this.lineWidth /= scl;
          },
   "matrix": function(m) {
